...
function CoreModificationState:new()
    local instance = setmetatable({}, CoreModificationState)
    instance.player = nil
...
function CoreModificationState:initializeBackgroundEffects()
    ...
    for i = 1, 50 do
        table.insert(self.backgroundParticles, {
            ...
            y = love.math.random(0, Config.nativeResolution.height),
    ...
    for i = 1, 8 do
        table.insert(self.dataStreams, {
            x = i * (Config.nativeResolution.width / 8),
...
function CoreModificationState:enter(player)
    print("Entered Enhanced CoreModificationState for player: " .. (player and player.name or "nil"))
    self.player = player
...
function CoreModificationState:triggerGlitchBurst()
    for _, effect in ipairs(self.glitchEffects) do
        if love.math.random() < 0.6 then
            effect.active = true
            effect.elapsed = 0
            effect.x = love.math.random(0, Config.nativeResolution.width)
            effect.y = love.math.random(0, Config.nativeResolution.height)
        end
    end
end
...
function CoreModificationState:drawEnhancedBackground()
    ...
    love.graphics.setColor(Config.activeColors.accent[1], Config.activeColors.accent[2], 
                          Config.activeColors.accent[3], 0.08)
    ...
    for _, stream in ipairs(self.dataStreams) do
        UIHelpers.drawDataStream(stream.x - 5, 0, 10, nativeH, 
                                ...
                                stream.density, Config.activeColors.accent)
    end
    ...
    if self.isTransitioning then
        ...
        love.graphics.setColor(Config.activeColors.accent[1], Config.activeColors.accent[2], 
                              Config.activeColors.accent[3], transitionAlpha * 0.3)
...
function CoreModificationState:draw()
    ...
    UIHelpers.drawTextWithGlow(resourceText, pX + pW/2, pY + 20, _G.Fonts.large, 
    ...
    if #self.availableMods == 0 then
        ...
        UIHelpers.drawHolographicText(">> ALL_SYSTEMS_OPTIMIZED: No further modifications available <<", 
                                      pX + pW/2 - _G.Fonts.medium:getWidth(">> ALL_SYSTEMS_OPTIMIZED: No further modifications available <<")/2, 
                                      currentContentY + 100, _G.Fonts.medium, 
...
function CoreModificationState:drawEnhancedSelectableList(items, selectedIndex, x, y, width, itemHeight, maxVisibleItems, scrollOffset)
    scrollOffset = scrollOffset or 0
    maxVisibleItems = maxVisibleItems or #items

    -- Enhanced background for the list area
    local listBgColor = {Config.activeColors.background[1], Config.activeColors.background[2], 
                        Config.activeColors.background[3], 0.6}
    ...
    for i = 1, maxVisibleItems do
        ...
        elseif item.disabled then
            ...
            love.graphics.setColor(Config.activeColors.text[1], Config.activeColors.text[2], 
                                  Config.activeColors.text[3], hoverAlpha)
            UIHelpers.drawRoundedRect(x - 4, currentY - 1, width + 8, itemHeight - 2, 2, "fill")
            ...
            if item.isAffordable then
                ...
                love.graphics.setColor(Config.activeColors.text_warning)
            end
        end
    ...
    if #items > maxVisibleItems then
        ...
        love.graphics.setColor(Config.activeColors.highlight[1] * thumbPulse, 
                              ...
                              Config.activeColors.highlight[3] * thumbPulse, 0.9)
...
function CoreModificationState:keypressed(key)
    ...
    elseif key == "down" then
        ...
        if self.selectedOption > self.scrollOffset + self.itemsPerPage then
            self.scrollOffset = math.min(#self.availableMods - self.itemsPerPage, self.selectedOption - self.itemsPerPage)
            self.scrollOffset = math.max(0, self.scrollOffset)
        end
        _G.SFX.play("ui_navigate")
        self:triggerSelectionEffect()
