...
function SubroutineChoiceState:new()
    local instance = setmetatable({}, SubroutineChoiceState)
    instance.player = nil
    instance.choices = {}
...
function SubroutineChoiceState:update(dt)
    ...
    for _, drop in ipairs(self.matrixRain) do
        ...
        -- Reset if off screen
        if drop.y > _G.Config.nativeResolution.height + drop.length * 15 then
            drop.y = -drop.length * 15
            drop.x = love.math.random(0, _G.Config.nativeResolution.width)
        end
    end
...
function SubroutineChoiceState:drawEnhancedBackground()
    ...
    for _, effect in ipairs(self.hologramEffects) do
        ...
        for i = 0, effect.height, 3 do
            ...
            love.graphics.setColor(_G.Config.activeColors.accent[1], _G.Config.activeColors.accent[2], 
                                  _G.Config.activeColors.accent[3], lineAlpha)
    ...
    if self.cacheAnalysis.active then
        for _, scanLine in ipairs(self.cacheAnalysis.scanLines) do
            ...
            love.graphics.setColor(_G.Config.activeColors.highlight[1], 
                                  _G.Config.activeColors.highlight[2], 
                                  _G.Config.activeColors.highlight[3], alpha)
            love.graphics.rectangle("fill", scanX, scanLine.y, 40, 2)
            
            -- Scan beam glow
            love.graphics.setColor(_G.Config.activeColors.highlight[1], 
                                  _G.Config.activeColors.highlight[2], 
                                  _G.Config.activeColors.highlight[3], alpha * 0.3)
    ...
    if self.isTransitioning then
        ...
        love.graphics.setColor(_G.Config.activeColors.accent[1], _G.Config.activeColors.accent[2], 
                              _G.Config.activeColors.accent[3], scanAlpha)
...
function SubroutineChoiceState:draw()
    ...
    UIHelpers.drawHolographicText(statusText, pX + pW/2 - _G.Fonts.medium:getWidth(statusText)/2, 
                                  pY + 15, _G.Fonts.medium, 
                                  {_G.Config.activeColors.accent[1] * statusPulse,
                                   _G.Config.activeColors.accent[2] * statusPulse,
                                   _G.Config.activeColors.accent[3] * statusPulse, 1}, 
                                  self.animationTime)
    ...
    if #self.displayListItems == 0 or (#self.displayListItems == 1 and self.displayListItems[1].type == "none") then
        ...
        UIHelpers.drawHolographicText(errorText, 
                                      pX + pW/2 - _G.Fonts.medium:getWidth(errorText)/2, 
                                      currentContentY + 80, _G.Fonts.medium, 
    ...
    local controlsColor = {_G.Config.activeColors.ui_text_dim[1] * controlsPulse,
                          _G.Config.activeColors.ui_text_dim[2] * controlsPulse,
                          _G.Config.activeColors.ui_text_dim[3] * controlsPulse, 1}
    
    UIHelpers.drawTextWithGlow("INTERFACE: ↑↓ Navigate | ENTER Integrate | ESC Abort", 
                               nativeW/2, controlsY, _G.Fonts.small, 
                               controlsColor, "center", 0.3)
end

function SubroutineChoiceState:drawEnhancedIntegrationList(items, selectedIndex, x, y, width, itemHeight, maxVisibleItems, scrollOffset)
    ...
    love.graphics.setColor(_G.Config.activeColors.highlight[1], _G.Config.activeColors.highlight[2], 
                          _G.Config.activeColors.highlight[3], chamberGlow * 0.4)
    ...
    for i = 1, maxVisibleItems do
        ...
        if isSelected then
            ...
            local highlightColor = _G.Config.activeColors.highlight
            ...
            love.graphics.print(">>", x - 30 - bracketOffset, currentY + itemHeight/2 - _G.Fonts.medium:getHeight()/2)
            love.graphics.print("<<", x + width + 12 + bracketOffset, currentY + itemHeight/2 - _G.Fonts.medium:getHeight()/2)
        ...
        elseif item.disabled then
            ...
            love.graphics.setColor(_G.Config.activeColors.text[1], _G.Config.activeColors.text[2], 
                                  _G.Config.activeColors.text[3], readyAlpha)
            UIHelpers.drawRoundedRect(x - 5, currentY - 2, width + 10, itemHeight - 4, 4, "fill")
            ...
            love.graphics.setColor(_G.Config.activeColors.accent[1], _G.Config.activeColors.accent[2], 
                                  _G.Config.activeColors.accent[3], readyPulse * 0.4)
        ...
        if not item.disabled then
            love.graphics.setFont(_G.Fonts.small)
            local typeColor = _G.Config.activeColors.accent
            if item.type == "new" then
                typeColor = _G.Config.activeColors.pickup
            ...
            elseif item.type == "upgrade" then
                typeColor = _G.Config.activeColors.highlight
                love.graphics.setColor(typeColor[1], typeColor[2], typeColor[3], 0.8)
    ...
    if #items > maxVisibleItems then
        ...
        love.graphics.setColor(_G.Config.activeColors.highlight[1] * thumbPulse, 
                              _G.Config.activeColors.highlight[2] * thumbPulse, 
                              _G.Config.activeColors.highlight[3] * thumbPulse, 0.95)
        ...
        for i = 0, 2 do
            ...
            love.graphics.setColor(_G.Config.activeColors.highlight[1], _G.Config.activeColors.highlight[2], 
                                  _G.Config.activeColors.highlight[3], indicatorAlpha)
...
function SubroutineChoiceState:keypressed(key)
        ...
        if chosenRawData and chosenRawData.type ~= "none" then
            if chosenRawData.type == "new" then
                ...
                if gameplay then 
                    ...
                    gameplay:logMessage("INTEGRATION_COMPLETE: " .. subDef.name, _G.Config.activeColors.pickup) 
                end
            elseif chosenRawData.type == "upgrade" then
                self.player:upgradeSubroutine(chosenRawData.subroutineInstance)
                if gameplay then 
                    gameplay:logMessage("UPGRADE_COMPLETE: " .. chosenRawData.subroutineInstance:getName(), _G.Config.activeColors.pickup) 
                end
            end
        else
            if gameplay then gameplay:logMessage("CACHE_ANALYSIS_FAILED: No compatible data found.", _G.Config.activeColors.text) end
        end
    ...
    elseif key == "escape" then
        _G.SFX.play("ui_back")
        local gameplay = GameState.get("gameplay")
        if gameplay then gameplay:logMessage("INTEGRATION_ABORTED: Cache remains sealed.", _G.Config.activeColors.text) end
        GameState.switch("gameplay")
        if gameplay and gameplay.resume then gameplay:resume() end
    end
    return true
...
function SubroutineChoiceState:triggerSelectionEffect()
    ...
    for i = 1, 5 do
        table.insert(self.dataFlowParticles, {
            x = _G.Config.nativeResolution.width / 2,
...
function SubroutineChoiceState:triggerIntegrationEffect()
    ...
    for i = 1, 15 do
        table.insert(self.dataFlowParticles, {
            ...
            y = _G.Config.nativeResolution.height / 2,
